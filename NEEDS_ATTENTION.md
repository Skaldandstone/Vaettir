...
- **P8-04 mobile push notifications need a real EAS project.** The server side (Expo push send, dead-token pruning, sign-off-request notifications) is real and verified against Expo's actual production endpoint. But generating an actual push token on a real device needs `apps/mobile/app.json`'s `extra.eas.projectId` - no EAS project has ever been created for this app, so that field genuinely doesn't exist (not omitted by mistake). To turn this on: run `eas init` (or create the project at https://expo.dev) under your own Expo account, which writes a real `projectId` into `app.json`, then a real device running the Expo Go client or a built app can register a token through the flow already wired in `App.tsx`.
