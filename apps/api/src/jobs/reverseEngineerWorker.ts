...
export async function runReverseEngineerJob(jobId: string): Promise<void> {
  ...
  try {
    ...
    const triggeringResult = job.triggeringResultId ? await validateTriggeringResult(prisma, job.projectId, job.triggeringResultId) : null;
    ...
    if (job.inputType === "CI_UNMATCHED_RESULT" && job.triggeringResultId) {
      ...
      if (match?.source && triggeringResult) {
        await linkExternalTestResult(prisma, { projectId: job.projectId, testResultId: triggeringResult.id, testCaseId: match.id, jobId: job.id });
      }
    }
    await prisma.reverseEngineerJob.update({ where: { id: job.id }, data: { status: "SUCCEEDED", completedAt: new Date() } });
  } catch (e) {
