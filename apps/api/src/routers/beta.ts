...
export const betaRouter = router({
  ...
  eligibility: protectedProcedure.query(async ({ ctx }) => {
    ...
    const canManageEnrollments = isStaffEmail(ctx.user.email);
    const enrollment = await ctx.prisma.betaEnrollment.findUnique({
      where: { email },
    });
    const status: "NOT_ENROLLED" | "REVOKED" | "CLAIMED" | "ELIGIBLE" =
      !enrollment
  ...
  list: staffProcedure.query(async ({ ctx }) => ({
    ...
    enrollments: await ctx.prisma.betaEnrollment.findMany({
  ...
  enroll: staffProcedure
    ...
    .mutation(({ ctx, input }) =>
      enrollBetaOwner(ctx.prisma, input.email, ctx.user.id, input.reason),
    ),
  enrollFromStudio: staffTokenProcedure
    .input(
      z
        .object({ email: z.string().email(), requestId: z.string().uuid() })
        .strict(),
    )
    .mutation(async ({ ctx, input }) => {
      ...
      if (enrollment.studioRequestId !== input.requestId) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "Studio request does not own this enrollment",
        });
      }
      return {
        id: enrollment.id,
  ...
  revokeFromStudio: staffTokenProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      const revoked = await revokeBetaEnrollment(
        ctx.prisma,
        input.enrollmentId,
        input.requestId,
      );
      if (!revoked.createdBy.startsWith("studio:")) {
        throw new TRPCError({
          code: "NOT_FOUND",
          message: "Studio enrollment not found",
        });
      }
