...
export async function projectMappings(tx: Prisma.TransactionClient, projectId: string, externalIds: string[]) {
  const stableIds = new Map(
    externalIds
      .filter((externalId) => /^VAE-[A-Za-z0-9_-]+$/.test(externalId))
      .map((externalId) => [externalId.slice(4), externalId] as const),
  );
  ...
  const stableCases = stableIds.size
    ? await tx.testCase.findMany({
        where: { projectId, id: { in: [...stableIds.keys()] } },
        select: { id: true },
      })
    : [];
  ...
  for (const source of sources) {
    ...
    if (mapping.has(source.externalTestId)) {
      throw new TRPCError({ code: "CONFLICT", message: "Ambiguous CI test mapping in this project. Resolve duplicate mappings before retrying." });
    }
    mapping.set(source.externalTestId, source.testCaseId);
  }
  for (const testCase of stableCases) {
    const externalId = stableIds.get(testCase.id)!;
    const prior = mapping.get(externalId);
    if (prior && prior !== testCase.id) {
      throw new TRPCError({ code: "CONFLICT", message: "The stable Vaettir id conflicts with a legacy CI mapping in this project." });
    }
    mapping.set(externalId, testCase.id);
  }
    ...
    if (result.externalTestId) {
      ...
      if (testCase.source) {
        ...
        if (!testCase.source.externalTestId) {
          await tx.testCaseSource.update({ where: { id: testCase.source.id }, data: { externalTestId: result.externalTestId } });
        }
      }
    }
