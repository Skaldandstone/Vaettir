...
beforeAll(async () => {
  ...
  const enrollment = await studioCaller().beta.enrollFromStudio({
    email: ownerEmail,
  ...
  const organization = await (
    await callerFor(ownerId)
  ).organization.bootstrap({ organizationName: `Beta ${run}` });
...
afterAll(async () => {
  ...
  if (enrollmentIds.length > 0) {
    await prisma.betaEnrollment.deleteMany({
...
describe("private-beta and Studio enrollment boundaries", () => {
  it("requires an enrollment, hides the tier, and prevents owner plan bypass", async () => {
    ...
    await expect(
      outsiderCaller.organization.bootstrap({ organizationName: "Uninvited" }),
    ...
    await expect(
      owner.organization.changePlanTier({
        organizationId,
        planTierId: publicTiers[0].id,
      }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
    await expect(
      owner.beta.enroll({ email: `${run}-blocked@example.com`, reason: run }),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  ...
  it("binds Studio retries and revocation to the immutable request id", async () => {
    ...
    const stored = await prisma.betaEnrollment.findUniqueOrThrow({
      where: { id: first.id },
    });
    ...
    await expect(
      studio.beta.enrollFromStudio({
        email: `${run}-other@example.com`,
        requestId,
      }),
    ).rejects.toMatchObject({ code: "CONFLICT" });
    await expect(
      studio.beta.enrollFromStudio({ email, requestId: randomUUID() }),
    ).rejects.toMatchObject({ code: "CONFLICT" });
    await expect(
      studio.beta.revokeFromStudio({
        enrollmentId: first.id,
        requestId: randomUUID(),
      }),
    ).rejects.toMatchObject({ code: "NOT_FOUND" });

    const revoked = await studio.beta.revokeFromStudio({
      enrollmentId: first.id,
      requestId,
    });
    expect(revoked.revokedAt).not.toBeNull();
    const repeated = await studio.beta.revokeFromStudio({
      enrollmentId: first.id,
      requestId,
    });
    ...
    await expect(
      anonymous.beta.enrollFromStudio({
        email: `${run}-denied@example.com`,
        requestId: randomUUID(),
      }),
  ...
  it("rejects a revoked enrollment at the server-side creation gate", async () => {
    ...
    enrollmentIds.push(enrollment.id);
    await studioCaller().beta.revokeFromStudio({
    ...
    await expect(
      bootstrapBetaOrganization(prisma, user, "Must not exist"),
    ).rejects.toMatchObject({ code: "FORBIDDEN" });
  ...
  it("reports an unclaimed reservation and staff self-service capability separately", async () => {
    ...
    enrollmentIds.push(enrollment.id);
    expect(
      await (await callerFor(eligibleUser.id)).beta.eligibility(),
    ).toMatchObject({
    ...
    const staffCaller = await callerFor(staff.id);
    expect(await staffCaller.beta.eligibility()).toMatchObject({
      email: staff.email,
      status: "ELIGIBLE",
      eligible: true,
      canManageEnrollments: true,
    });
    const staffOrganization = await staffCaller.organization.bootstrap({
      organizationName: `Owner ${run}`,
    });
    organizationIds.push(staffOrganization.id);
    const staffEnrollment = await prisma.betaEnrollment.findUniqueOrThrow({
      where: { email: staff.email },
    });
    enrollmentIds.push(staffEnrollment.id);
    expect(staffEnrollment).toMatchObject({
      createdBy: `owner:${staff.id}`,
      reason: "Automatic full-access owner onboarding",
      organizationId: staffOrganization.id,
    });
    expect(staffEnrollment.claimedAt).not.toBeNull();
  });
  ...
  it("serializes pending invitations against private-beta seat limits", async () => {
    ...
    const fullInvitations = await Promise.allSettled(
      Array.from({ length: 9 }, (_, index) =>
        owner.organization.inviteMember({
          organizationId,
    ...
    expect(
      fullInvitations.filter((outcome) => outcome.status === "fulfilled"),
    ).toHaveLength(4);
    ...
    expect(
      readOnlyInvitations.filter((outcome) => outcome.status === "fulfilled"),
    ).toHaveLength(2);
    ...
    const acceptances = await Promise.allSettled([
      (await callerFor(invitee.id)).organization.acceptInvitation({
    ...
    expect(
      acceptances.filter((outcome) => outcome.status === "fulfilled"),
    ).toHaveLength(1);
    expect(
      await prisma.membership.count({
        where: { organizationId, seatType: "FULL" },
      }),
    ).toBe(2);
    expect(
