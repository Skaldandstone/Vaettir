...
export async function enrollBetaOwner(
  ...
  return db.$transaction(async (tx) => {
    ...
    if (studioRequestId) {
      const requestEnrollment = await tx.betaEnrollment.findUnique({ where: { studioRequestId } });
      if (requestEnrollment && requestEnrollment.email !== normalized) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "This Studio request is already bound to a different email address",
        });
      }
      if (requestEnrollment && !requestEnrollment.revokedAt) return requestEnrollment;
    }
    ...
    if (existing && !existing.revokedAt) {
      if (studioRequestId && existing.studioRequestId !== studioRequestId) {
        throw new TRPCError({
          code: "CONFLICT",
          message: "This email already has a reservation from a different Studio request",
        });
      }
      return existing;
    }
    if (existing?.claimedAt) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "This enrollment was already claimed. Reactivate the existing organization through support.",
      });
    }
    ...
    if (reserved >= BETA_TEAM_LIMIT) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "The three-team private beta is full. Revoke an unused enrollment before inviting another team.",
      });
    }
...
export async function revokeBetaEnrollment(db: PrismaClient, id: string, expectedStudioRequestId?: string) {
  return db.$transaction(async (tx) => {
    await tx.$queryRaw`SELECT "id" FROM "PlanTier" WHERE "key" = ${PRIVATE_BETA_TIER} FOR UPDATE`;
    ...
    if (!enrollment || (expectedStudioRequestId && enrollment.studioRequestId !== expectedStudioRequestId)) {
      throw new TRPCError({ code: "NOT_FOUND", message: "Beta enrollment not found" });
    }
    if (enrollment.claimedAt) {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: "Claimed enrollments cannot be revoked. Suspend the organization through support instead.",
      });
    }
...
export async function bootstrapBetaOrganization(
  ...
  return db.$transaction(async (tx) => {
    ...
    if (!enrollment || enrollment.revokedAt || enrollment.claimedAt) {
      throw new TRPCError({
        code: "FORBIDDEN",
        message: "Vaettir is invite-only. Ask support for a beta owner invitation, or use your team's membership invitation.",
      });
    }
    ...
    await tx.auditLog.create({
      data: {
        ...
        action: "CREATE",
        summary: "Private beta invitation accepted; five full seats, two read-only seats, 500 monthly AI credits.",
      },
    });
    return organization;
  });
