...
const test = browserTest.extend({
  ...
  clerkTestToken: async ({}, runFixture) => { await runFixture(); },
});

test("member fixture contains deterministic run, coverage, healing and mapped control data", async ({ isolatedOrg }) => {
...
test.describe("read-only fixture", () => {
  ...
  test("has a separate full owner and read-only invited user", async ({ isolatedOrg }) => {
...
test.describe("invitation fixture", () => {
  ...
  test("recipient has no membership before accepting", async ({ isolatedOrg }) => {
    ...
    expect(invite.email).toBe(process.env.CLERK_TEST_EMAIL);
...
test.describe("enrolled fixture", () => {
  ...
  test("can bootstrap a new beta organization and cleans it afterwards", async ({ isolatedOrg }) => {
...
test.afterAll(async () => {
  ...
  expect(await prisma.user.count({ where: { OR: [{ clerkUserId: process.env.CLERK_TEST_USER_ID }, { clerkUserId: { startsWith: "e2e-owner-" } }] } })).toBe(0);
  expect(await prisma.complianceControl.count({ where: { code: { startsWith: "E2E-" } } })).toBe(0);
