export function assertTestEnvironment(env: Record<string, string | undefined> = process.env) {
  ...
  if (!["postgresql:", "postgres:"].includes(db.protocol) || !["localhost", "127.0.0.1", "postgres"].includes(db.hostname) || !/^\/vaettir_[a-z0-9_]*test$/.test(db.pathname)
    || [...db.searchParams.keys()].some((key) => !["schema", "connection_limit", "pool_timeout"].includes(key))) {
    throw new Error("E2E requires an isolated local vaettir_*test database. Shared and production databases are forbidden.");
  }
  if (![web, api].every((url) => ["http:", "https:"].includes(url.protocol) && ["localhost", "127.0.0.1"].includes(url.hostname) && !url.username && !url.password)) {
