...
- **Prisma migrations run as the container's own startup command** (`prisma migrate deploy && tsx src/server.ts` in `Dockerfile.api`'s `CMD`), not as a separate CI step — RDS isn't reachable from outside the VPC, so the running ECS task is the only thing with network access to migrate against. Same reasoning as Kall's `alembic upgrade head && uvicorn ...`.
- **`Dockerfile.api` runs via `tsx src/server.ts`, not a compiled `node dist/server.js`.** `packages/db` and `packages/core` are TS-source-only (`package.json` `"main": "src/index.ts"`); a plain-node runtime fails resolving their relative `.js`-suffixed imports (only the `.ts` files exist). Fixing that properly means giving those packages a real build step — not done yet, tracked as a gap below.
...
- **`packages/db`/`packages/core` have no real build step**, which is why `Dockerfile.api` runs via `tsx` instead of compiled JS. Worth fixing properly at some point (add `tsc` builds + point `main`/`types` at `dist/`) so the API can run as compiled JS like a normal production Node service — not urgent since `tsx` works fine, just non-standard.
