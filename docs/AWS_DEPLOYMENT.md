...
| CodeBuild projects | `vaettir-api-build` (`Dockerfile.api`), `vaettir-web-build` (`Dockerfile.web`, env vars `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY`/`NEXT_PUBLIC_API_URL`/`NEXT_PUBLIC_SENTRY_DSN` baked in on the project, `VAETTIR_RELEASE_COMMIT` overridden per-build by `deploy-aws.sh`) - both source from S3 with an inline buildspec (`docker build` + `docker push`, `privilegedMode: true`), `aws/codebuild/standard:7.0` |
