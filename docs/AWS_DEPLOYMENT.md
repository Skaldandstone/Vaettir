...
- **Browser cross-origin access is exact-allowlist only**: same-origin requests through `vaettir.skaldandstone.com` need no CORS header. Set `WEB_APP_URL` for the canonical browser origin and use comma-separated `CORS_ALLOWED_ORIGINS` only for deliberate additional browser origins. Production does not allow wildcards or implicit localhost origins. Requests without an `Origin` header remain available to health probes, signed webhooks, native clients, and server-to-server callers.
- **Privileged access denials emit privacy-safe structured warnings**: staff-email, trusted-service-token, and live-app-scan allowlist failures log only a fixed security event, surface, reason, and header-presence booleans. They do not log token, actor, email, user, tenant, request-body, or customer-data values. Deployment still needs a retained CloudWatch query/alert and operator acceptance before this counts as an operational control.
...
| ECS services              | `vaettir-api` (Service Connect _and_ public ALB path at `/api/*`), `vaettir-web` (behind the ALB)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| Security groups           | `vaettir-alb-sg` (80 from internet) → `vaettir-web-tasks-sg` (3000 from ALB) and `vaettir-api-tasks-sg` (8000 from web tasks _and_ from `vaettir-alb-sg` directly) → `vaettir-rds-sg` (5432 from API tasks)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
