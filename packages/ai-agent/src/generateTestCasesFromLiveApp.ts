...
const EMIT_TEST_CASES_TOOL: Anthropic.Tool = {
  ...
  description: "Emit BDD test case(s) grounded in what was actually observed on the live app.",
  input_schema: {
    type: "object",
    properties: {
      testCases: {
        ...
        items: {
          ...
          properties: {
            ...
            testType: {
              type: "string",
              enum: ["UNIT", "FUNCTIONAL", "CONTRACT", "INSTRUMENTATION", "SMOKE", "SANITY", "REGRESSION", "E2E", "PERFORMANCE", "SECURITY", "ACCESSIBILITY", "EXPLORATORY", "COMPLIANCE", "OTHER"],
            },
...
a web application, grounded ONLY in the pages and interactive elements a
real crawl actually observed on the live site.
...
- Prefer flows that span multiple observed pages when the crawl data
  shows a plausible connection (e.g. a link from page A's elements leads
  to page B), since that's the kind of test a live-app scan can uniquely
...
function buildPromptContent(input: GenerateTestCasesFromLiveAppInput): string {
  const pagesText = input.pages
    ...
    .join("\n\n---\n\n");
  return `Live-app crawl starting at: ${input.startUrl}\n\n${pagesText}\n\nGenerate BDD test cases grounded only in the pages and elements above.`;
}
...
export async function generateTestCasesFromLiveApp(
  ...
  const message = await traceAnthropicCall("generateTestCasesFromLiveApp", MODEL, () =>
